...
beforeEach( async () => {
    ...
    const Health = await ethers.getContractFactory("Health");
    contract = await Health.deploy();
...
describe("Health Contract Test", () => {
    ...
    it("patients can share to doctors", async () => {
        await contract.connect(accounts[12]).shareToDoctor(accounts[8].address);
        const data = await contract.connect(accounts[8]).getMyPatients();
        assert.equal(data[0], accounts[12].address);
    })
